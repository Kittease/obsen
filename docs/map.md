...
Found by query, not listed here — open tickets in `docs/tickets/` with all blockers closed and no assignee. Everything but the last two design steps is resolved. [Design the settings/onboarding UX](tickets/022-design-settings-onboarding-ux.md) is the frontier; then only [Write the Obsen v1 spec](tickets/023-write-v1-spec.md) remains.
