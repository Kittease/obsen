...
- [On-device spike](tickets/019-prototype-on-device-spike.md) — SDK-with-patches confirmed on a real Android phone: login 2.8 s, list ~50–160 ms, 8 MiB E2EE round-trip byte-identical (~2.5 MiB/s up, ~5.3 MiB/s down), auth restore across app kill, `visibilitychange` reliable both ways; engine must make sync runs single-flight (feeds 021); iOS untested; huge-attachment memory question stays on the 014 watch list.
- [Sync-state schema and change detection](tickets/020-design-sync-state-schema.md) — SHA-512 everywhere (Filen's own plaintext content hash — the ticket's xxhash premise was wrong); mtime+size cheap path with hash-on-mismatch; six-field records keyed by path in a dedicated `sync-state.json` (atomic tmp+rename; `remoteUuid` doubles as remote change detector since Filen content updates mint new UUIDs); corrupt/lost/downgraded/re-linked state degrades to first-link re-bootstrap; scope-predicate contract keeps selective sync unblocked; `mergeable` = {`.md`,`.txt`} engine constant.
- [Sync engine algorithm](tickets/021-design-sync-engine-algorithm.md) — single path (every trigger = mark dirty + request run), single-flight scheduler with coalescing pending scope; full remote listing every run with UUID scope-expansion; decision matrix where first link = reconcile with empty state (no bootstrap module); three-tier rename pairing (UUID / live hints / exact-hash); five execution phases, deletes last, transfer concurrency 4; journal-less crash recovery (every op redo-safe — the matrix is the mechanism); precise self-echo filters with idempotence as the guarantee; offline backoff 10 s→5 m, skip-and-surface for unmaterializable names, NFC at the port boundary; three ports (`VaultPort`/`RemotePort`/`StorePort`) finalized as TS signatures; engine status enum is 022's contract.

## Frontier / open tickets

Found by query, not listed here — open tickets in `docs/tickets/` with all blockers closed and no assignee. All research, prototype, and engine-design tickets are resolved. [024 choose license](tickets/024-choose-license.md) and [025 adopt SecretStorage?](tickets/025-adopt-secretstorage-for-credentials.md) are unblocked and parallelizable; [022 settings/onboarding UX](tickets/022-design-settings-onboarding-ux.md) waits on 025; then only [023 the spec itself](tickets/023-write-v1-spec.md) remains.
...
- **Sync status / activity UI specifics** — how the engine's status enum (`idle|syncing|offline|quota|auth-error|frozen`, per [021](tickets/021-design-sync-engine-algorithm.md)) is presented: status-bar states, notices, log format, auth-expiry re-prompt flow. Sharpens inside the UX design ticket.
